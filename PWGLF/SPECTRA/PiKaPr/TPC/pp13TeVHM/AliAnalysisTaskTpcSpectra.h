...
class AliAnalysisTaskTpcSpectra : public AliAnalysisTaskSE
{
 ...
 private:
  ...
  //
  //Utility functions
  Float_t GetCombinedSigma(AliAODTrack* track, AliPID::EParticleType type) { return TMath::Sqrt(TMath::Power(fPIDResponse->NumberOfSigmasTPC(track, type), 2) + TMath::Power(fPIDResponse->NumberOfSigmasTOF(track, type), 2)); }
  //
